...
namespace Roads.Common.Repositories
{
    ...
    public class RoutesRepository : RepositoryBase
    {
        ...
        /// </returns>
		public RoutesSearchResultData GetRoutesSearchResultData( int startedCityId, int destinationCityId, int page, string languageName, bool isRouteValidation )
        {
            var recordsPerPage = GetNumberOfRecordsPerPage();

            var count = DataContext
                .Treks.Count(w => w.OriginCityNodeId == startedCityId && w.DesinationCityNodeId == destinationCityId);
            ...
            if (count != 0)
            {
                ...
                var traks = DataContext
                    .Treks
                    .Where(w => w.OriginCityNodeId == startedCityId && w.DesinationCityNodeId == destinationCityId)
